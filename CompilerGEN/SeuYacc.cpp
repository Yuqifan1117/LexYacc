...
bool SeuYacc::readGrammer(const std::string grammerFile) {
	...
	IDType state = 0;
	std::string lexeme = ""; //根据lex词法保存一个单词（一个非终结符或终结符）
	IDType procHead = 9999;
	bool endFlag = false;
	unsigned int braceMatch = 0; //用于匹配括号

	_indexSymbol = 0;
	std::list<ElementType> list; // 利用list保存产生式，类型为一个element（终结符或非终结符）
	...
	while (!endFlag) {
		...
		case 0:
			...
			if (!read()) endFlag = true;
				...
				if (found != _nonterminalTable.end()) {
					...
					_nonterminalNullable.insert(std::make_pair(_indexSymbol, false));// 默认新保存的非终结符不产生空串
		...
		case 1:
			...
			if (isLegalLetter(_peek)) {
				...
				if (list.empty()) {
					// empty production，产生式为空串，记录该非终结符会产生空串
					_nonterminalNullable.find(procHead)->second = true;
				}
				list.swap(std::list<ElementType>());// 清空list列表
				lexeme = "";
			}
			else if (_peek == '\'' || _peek == ';' || _peek == '{') {
				if (!lexeme.empty()) {
	...
	_terminalTable.insert(std::make_pair("$", _indexSymbol)); //$作为特殊的终结符
	_word.push_back("$");
	++_indexSymbol;
		...
		if (_peek == 'l') {
			...
			while (_peek != '%') {
				if (_peek == ' ' || _peek == '\t') {}
					...
					_terminalTable.insert(std::make_pair(lexeme, _indexSymbol)); //终结符表保存操作符和index（操作符序号）
					_priority.insert(std::make_pair(_indexSymbol, curPriority)); //优先级表保存操作符index和优先级
					_leftAssociative.insert(std::make_pair(_indexSymbol, true));	//左递归表保存操作符index和满足左递归
...
void SeuYacc::initTransition() {
	...
	H.addItem(LR1Item(&*_productionVector.begin(), predSet));  // S'->S，$
	_LR1stateTransition.push_back(TransitionItem(Closure(H), 0)); // 产生相应的闭包
	// 状态边的扩展
			...
			if (!newState.itemList.empty()) {
				...
				for (auto& m : _LR1stateTransition) {
					if (m.LR1state.equal(newState)) {
						...
						_LR1stateTransition.at(k).TransMap.insert(std::make_pair(edge, m.index));
						newStateFlag = false;
						break;
					}
				}
				if (newStateFlag) {
					...
					_LR1stateTransition.at(k).TransMap.insert(std::make_pair(edge, maxState));
					++maxState;
				}
			}
		}
			...
			if (it2._head == *(A._dot)) {
				...
				if (next == A._prod->_bodyList.end()) {
					...
					if (found == _nonterminalNullable.end()) {
						...
						std::set<ElementType> firstC = First(*next); //得到C的FIRST集合
	...
	if (!_LALR1parseTable.empty()) {
		for (auto& item : _LALR1parseTable) {
			...
			for (ElementType edge = 0; edge < _indexSymbol; ++edge) {
				...
				case SHIFT:
					file << _word.at(edge) << " S " << item.second[edge].index << std::endl; break;
				case REDUCTION:
					file << _word.at(edge) << " R " << item.second[edge].index << std::endl; break;
				case ACCEPT:
					file << _word.at(edge) << " accept" << std::endl; break;
				case GOTO_STATE:
					file << _word.at(edge) << " G " << item.second[edge].index << std::endl; break;
				}
			}
			file << std::endl;
		}
		...
		for (auto& item : _LR1parseTable) {
			...
			for (ElementType edge = 0; edge < _indexSymbol; ++edge) {
				...
				case SHIFT:
					file << _word.at(edge) << " S " << item[edge].index << std::endl; break;
				case REDUCTION:
					file << _word.at(edge) << " R " << item[edge].index << std::endl; break;
				case ACCEPT:
					file << _word.at(edge) << " accept" << std::endl; break;
				case GOTO_STATE:
					file << _word.at(edge) << " G " << item[edge].index << std::endl; break;
				}
			}
			file << std::endl;
		}
